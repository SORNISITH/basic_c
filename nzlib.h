#ifdef REVERSE_IMPLEMENT
#include <stdio.h>
#include <stdlib.h>
char* Reverse_Str(const char* arr, size_t len)
{
    if (len < 1) {
        printf("Error len \n");
        return NULL;
    }
    char* rev_chars = malloc(len + 1);
    const char *low = arr, *top = arr + len - 1;
    char *nlow = rev_chars, *ntop = rev_chars + len - 1;
...
#ifdef LINKLISTED_STR_IMPLEMENT
